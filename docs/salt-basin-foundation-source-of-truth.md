...
**Monetary River System → Basin → Ports → Source Observations / Evidence Currents → Elements → Evidence Atoms → Semantic Affinity Fields → Atom Clusters → Semantic Compositions / Molecules → Projection-relative Settlement → Riverbeds → Highways → Channels → Streams → Channel Rods → Nested Tributaries → Tributary Branches → Journey Positions → Journey Branches → Stages → Gates → Currents → Current Arcs → Confluences → Query Channels / Compressed Query Rods → Accounting Projection → Reciprocal Economic-Accounting State Inference → Heterosemantic Divergence → Policy-bounded Agents / Channel Rod Staff**

*Highway, Stream, Nested Tributary, and Tributary Branch added 2026-07-29 from `Salt_Basin_Expanded_Semantic_Scenario_and_Business_Instance_Model_v0.2.xlsx`. Monetary River System, Stage, Gate, Journey Position, and Journey Branch added the same day from the more precise, column-level `Salt_Basin_HOS_Canonical_Ontology_and_MRS_Workbook_v1.xlsx`, which also resolved two open questions — see below and the registry for full provenance. **One question remains genuinely open, not resolved:** the new workbook's "Current" (a directed movement segment/edge between Stages) does not match this document's own 2026-07-27 "Current" (a rule set governing entry/exit/maturity progression) — flagged in the registry, needs a direct Betsy decision before either is built against.*
...
- **Semantic Affinity Field / Bond:** contextual rule system that governs Atom attraction, repulsion, relevance, and admissibility — the real bonding-rule mechanism (tag-based attraction/overlap, computed dynamically), not a static pre-declared list. Implemented server-side in `server/lib/eidosBonding.js`, ported from the working client-side prototype `src/lib/journeyEngine/bonding.js`. Formalized 2026-07-29 by the MRS ontology workbook's `BOND_DEFINITION`/`BOND_ENDPOINT`/`BOND_INSTANCE`/`BOND_RULE` — same real mechanism, adding a typed vocabulary (association/composition/attribution/ownership/denomination/dependency/lineage/evidence/transition/allocation) and directionality the shipped bonding engines don't yet distinguish. Not built into code this pass — flagged as the real schema a future bonding-engine pass should build against.
...
- **Monetary River System (MRS):** the complete navigable network through which monetary value, rights, obligations, and evidence flow — contains Highways. Per `Salt_Basin_HOS_Canonical_Ontology_and_MRS_Workbook_v1.xlsx` (Betsy-supplied, 2026-07-29), now the canonical column-level source for the Network and Runtime Journey layers.
- **Highway:** canonical end-to-end value pathway, contained by an MRS and containing Channels (a Channel may participate in more than one Highway if governed). Carries a Value_Flow_Type (commercial, capital, revenue, cash, debt, asset, service, operational, risk, evidence, etc.). Per Betsy 2026-07-29: "Highway is the end to end process flow governance (was previously defined as the channel rod)" — the real, live, governing role earlier loosely attributed to Channel Rod is now formally Highway's, sitting one level above Channel as the cross-Channel domain governor. This does **not** change Channel Rod's own 2026-07-16 definition below. **Naming collision RESOLVED 2026-07-29:** "Salt Basin Highways™" (`docs/canon/SB-25-ROADMAP-DEPENDENCY-READINESS.md` — Route Intelligence Engine, Eligibility Engine, Monetary River System, etc., previously "architecture-only, nothing built") is not a separate, colliding meaning — it *is* this MRS/Highway model, now getting real schema. Not a rename fight; the trademark and the architecture object are the same thing.
- **Channel:** the *definition* of a temporal-destination corridor type — a governed process route within a Highway — governs what kind of semantic state can be projected, positioned, diverted, recombined, and interpreted along corridors of that type (e.g. Member, Customer, Revenue Lifecycle, Fund Deal, Portfolio Company, Value Creation). A Channel is declared once per type and carries no live state of its own. Backed by `journey_rod_types` (`server/db.js`).
- **Stream:** a scenario-specific configured pathway within a Channel (business model, segment, product, geography, transaction, or variant context), gated by an activation rule. A Channel contains many Streams; a Stream contains many Currents. **Placement question RESOLVED 2026-07-29:** the MRS workbook's Journey Position entity requires Highway+Channel but marks Stream and Current both *optional* — Stream is a scenario-specific refinement (matches its own examples: Amendment Configuration Stream, Renewal Configuration Stream), not a mandatory layer every journey passes through.
- **Channel Rod / Channel Journey:** the persistent *instance* of a Channel definition — one real, live occurrence of that Channel type for a specific entity, carrying actual temporal state, evidence, and settlement. Every `journey_data_rods` row is a Channel Rod instantiating its `rod_type`'s Channel definition. (Decided 2026-07-16, superseding the earlier "reference axis of a Channel" phrasing.) Confirmed 2026-07-29 as the same object the MRS ontology calls **Channel Journey** and the 2026-07-27 Career session's own terminology — one real object, multiple document-local names, not a conflict.
- **Journey Position:** bitemporal historical/current/planned/projected/simulated/alternative location of a Channel Journey — Highway+Channel required, Stream/Current/Stage optional, plus entry/exit/recorded timestamps and a confidence score for projected positions. The column-level mechanism this document's Current Arc concept already gestured at.
- **Journey Branch:** a governed branch from a parent Channel Journey to a new child Channel Journey (branch reason, merge status). The 1:1 lineage-fork view — **not** the same thing as Tributary Branch below (the N:M coordination-context view); both are real, kept distinct.
- **Stage:** a defined state or position in a Channel pathway, with hierarchy depth and an optional parent stage. A Highway may declare a canonical start/end Stage. **Not yet reconciled** against this document's own Current definition below, whose 2026-07-27 wording already used "stage" informally as part of its rule-set — flagged, not assumed identical.
- **Gate:** an entry, exit, approval, validation, security, evidence, financial, contractual, or temporal control, evaluated via a rule. Formalizes the "Convergence / Gate" vocabulary Salt Basin scenario definitions already use informally.
- **Nested Tributary / Tributary Connection:** the *definition* side of a bounded scenario-specific state-flow relationship — branch type, entry points, gates, and convergence rules — defined by a Highway, Channel, or Stream and invoked (never owned) by a Current. Formalizes what this document previously called "Channel Tributary." Refined 2026-07-29 by the MRS workbook's more precise `TRIBUTARY_CONNECTION` entity (a governed connection between two Currents, with a typed connection kind and an explicit context-transfer policy) — same concept, more precise name, kept as an alias.
- **Tributary Branch:** the persistent *instance* of a Nested Tributary/Tributary Connection — coordination authority, locks, participant references, and confluence history for one real branch (N:M — many journeys/channels connected together). Created or joined when a Current invokes a Nested Tributary; persists after confluence. Only branches with this semantic behavior are Tributaries. The one mechanism for every Tributary connection is `server/lib/tributaryRegistry.js`'s `TRIBUTARY_TYPES` registry — a new Tributary is a new registry entry, never bespoke insert code.
...
- **Agent Hierarchy:** self-referencing Agent tree (`Parent_Agent_ID`, `Agent_Type`: Executive Data Staff | Domain | Journey | Molecule | Atom | Element | Source Governance | Security | User Proxy | Lineage), each node carrying its own permitted-source, permitted-transformation, memory-retention, exposure, and action-permission policy; `AGENT_ASSIGNMENT` binds an Agent to any governed object with a role (primary/validator/approver/observer/lineage/security/source). Per `09_Rules_Agents_Security` of `Salt_Basin_HOS_Canonical_Ontology_and_MRS_Workbook_v1.xlsx` (2026-07-29). **This is the real column-level design for the multi-tier Context Orchestrator → Domain Agents → Source Agents hierarchy Loop 8 (Context) flagged as "not built" on 2026-07-12** — BestyStaff today is a single flat agent with no parent/child structure. Not built this pass; a future Loop 8/6 build should target this schema rather than inventing a new one.
