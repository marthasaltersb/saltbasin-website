...
export const ARTIFACT = {
  ...
  watermarkTemplate: '© 2026 Martha Elizabeth Salter - The Salter Influence LLC All Rights Reserved. · Contribution Intelligence Methodology™ · Master Enterprise Solution Best Bets™ · Licensed to: {{licensee}} · {{date}}',
};
...
export const RETROACTIVE_SESSION_ANALYSIS = {
  ...
  knownGaps: [
    ...
    'Full backlog reconciliation completed 2026-07-07 (v0.18.2). 17 items (81-91, 92-97) corrected via Tier 1 (real JSONL burst analysis). 41 items (originally 1-47, minus 3 merged epics) corrected via Tier 2 (coarse estimate: completedTurns / platform-average density, since their real session transcript — "Website admin panel and configurability," 2026-06-03->06-07, confirmed via the Claude Desktop session index — no longer exists; Desktop flags it transcriptUnavailable:true). 3 items (#1, #6, #10) retired as duplicate "epic" rollups of other items\' task ranges. 40 items (37-39, 48-80, 98-101) correctly untouched at 0/null — status=pending, not yet built. 9 items (102-110) corrected 2026-07-07 (v0.18.1). Tier 2\'s evidence is weaker than Tier 1: no burst/duration/human-vs-assistant split exists for those 41 items, only a total turn count, so their hours are directionally correct rather than independently verified.',
    'The 2026-06-03->06-07 transcript loss mechanism (retention policy, rotation, crash) was not investigated — if it recurs, active session data could be lost again without warning. The Claude Desktop session index (AppData/Roaming/Claude/claude-code-sessions) is a secondary metadata source worth checking first if a future gap appears — it can retain sessionId/cwd/title/completedTurns even after a transcript is gone.',
    'Content-level duplicate detection was not run — only externalRef task-number overlap was checked (which caught 3 items). Other backlog items may describe overlapping work without a shared task-number trail.',
    'The auth `sessions` table (server/db.js, used for login cookies) and the analytics session-tracking table added in v0.17 share the same name — the v0.17 CREATE TABLE was a silent no-op against the existing auth table. The naming collision itself is still unresolved (a proper fix means renaming/splitting a live auth table — flag to Betsy before touching it), but its downstream symptom — a broken FK in the backlog_items column migration that silently blocked data_source/est_*/actual_*/fee_type from ever being created — was fixed in v0.18.3 by dropping the FK and isolating every migration statement so one failure can\'t cascade.',
    'Cost figures (v0.18.3): costUsdClaude and traditionalCostUsd were computed from two disconnected legacy formulas (timeMinutes × $0.02/min and timeMinutes × $6.25/min) never tied to RATE_CONFIGS_2026. Recomputed across all 110 items as hoursClaude × $115/hr and (hoursClaude+hoursBetsy) × $175/hr. traditionalCostUsd assumes a traditional team takes the same wall-clock duration as the AI-augmented build — a floor comparison, not an independently-estimated engineer-equivalent-effort figure (still the unresolved gap noted above).',
