...
export const METRIC_DEFINITION_REGISTRY = Object.freeze({
  ...
  CHANNEL_MATURITY: {
    metricId: 'channel_maturity', metricKey: 'CHANNEL_MATURITY', displayName: 'Channel Maturity',
    // ROD_MATURITY was this metric's key through rod-mathematics-v1. Kept as an
    // alias rather than deleted, matching worldRegistry.js's own Journey ->
    // Channel handling (`canonicalId` + `legacyAliases`).
    legacyAliases: ['ROD_MATURITY'],
    metricCategory: C.MATURITY.categoryId,
    definition: 'How far a Channel Rod has matured toward each configured PURPOSE — legal, participation coverage, billing readiness, revenue-recognition readiness, renewal stability, expected realization. Purpose-, stage-, dependency-, and network-aware.',
    businessQuestionAnswered: 'Mature enough for what, exactly?',
    calculationMethod: 'independent per-purpose scores with dependency capping',
    formulaDescription: 'Each purpose is scored 0-1 from its own required clusters/atoms, then capped at dependencyCapFactor x its weakest declared dependency — billing readiness can never exceed legal maturity, because you cannot bill against an unsigned contract. Purposes with networkScope "related" also draw on sibling and child rods, since participation coverage is not knowable from one rod alone.',
    inputDimensions: ['legal', 'participation_coverage', 'billing_readiness', 'rev_rec_readiness', 'renewal_stability', 'expected_realization'],
    rangeMin: 0, rangeMax: 1, unit: 'score_0_1_per_purpose', higherIsBetter: true,
    directionalityDescription: 'There is NO single overall maturity score, by design. A signed master contract may sit at 100% legal maturity while participation coverage, billing readiness, rev-rec readiness, renewal stability and expected realization are all well below 100%. Any surface that renders one headline maturity number must name which purpose it means. Maturity is also strictly separate from STATE_CONFIDENCE: a fully mature purpose can still be low-confidence if the evidence behind it is stale, contradictory, or weakly sourced.',
    confidenceSupported: true, calculationVersion: 'rod-mathematics-v2', implementationStatus: 'calculated_not_rendered',
    visualEncoding: 'rod_crystal_complexity', allowedScopeTypes: ['evidence_atom', 'atom_cluster', 'journey_rod'],
    drilldownType: 'purpose_breakdown', effectiveFrom: '2026-07-29', effectiveTo: null,
  },
  STATE_CONFIDENCE: {
    metricId: 'state_confidence', metricKey: 'STATE_CONFIDENCE', displayName: 'State Confidence',
    metricCategory: C.CONFIDENCE.categoryId,
    definition: 'How strongly the current semantic state can be trusted, independent of how mature it is — from evidence quality, completeness, recency, consistency, authority, dependency resolution, and scenario-specific risk.',
    businessQuestionAnswered: 'How much should we trust what this says right now?',
    calculationMethod: 'weighted composite of six inputs, less a scenario risk penalty',
    formulaDescription: 'Weighted sum of evidence quality, completeness, recency, consistency, authority and dependency resolution (weights sum to 1), then MINUS scenarioRisk x maxPenalty. Risk is subtracted rather than weighted in so that a high-risk scenario can never raise confidence. Recency decays on a configurable half-life.',
    inputDimensions: ['evidenceQuality', 'completeness', 'recency', 'consistency', 'authority', 'dependencyResolution', 'scenarioRisk'],
    rangeMin: 0, rangeMax: 1, unit: 'score_0_1', higherIsBetter: true,
    directionalityDescription: 'Separate axis from CHANNEL_MATURITY and must never be blended into it. Distinct from QUERY_CONFIDENCE, which scores confidence in an answer to an active query; STATE_CONFIDENCE scores the underlying state itself and exists with no query context at all.',
    confidenceSupported: false, calculationVersion: 'rod-mathematics-v2', implementationStatus: 'calculated_not_rendered',
    visualEncoding: null, allowedScopeTypes: ['evidence_atom', 'atom_cluster', 'journey_rod'],
    drilldownType: 'dimension_breakdown', effectiveFrom: '2026-07-29', effectiveTo: null,
  },
  JOURNEY_DENSITY: {
