...
function EdgeCard({ card, collapsed, onToggle }) {
  return <article className={`mco-edge-card${collapsed ? ' collapsed' : ''}`}><button type="button" onClick={onToggle} aria-label={`${collapsed ? 'Expand' : 'Collapse'} ${card.label}`}>{collapsed ? '+' : '-'}</button><span>{card.label}</span><strong>{card.value}</strong>{!collapsed && <p>{card.detail}</p>}</article>;
}
...
function WorldCity({ world, activeJourney, setActiveJourney, activeVariant, setActiveVariant, onBack, onLaunch }) {
  ...
  return <section className={`mco-city-scene${activeJourney ? ' journey-active' : ''}`} style={{ '--accent': world.accent }}>
    ...
    <div className="mco-city-sky" /><header><button type="button" onClick={onBack}>&lt; All worlds</button><div><span>{world.shortLabel} world</span><h1>{world.label}</h1><p>{world.description}</p></div><HealthPill /></header>
