...
### Spatial Journey World ↔ member journeys

`SpatialJourneyWorld.jsx` (the `/output/business-definition-experience` world, also embedded in `LonetreeMvpPanel`) renders the signed-in user's own scenario-driven `journey_data_rods` alongside its seeded reference rods (connected 2026-09-23). Data comes from `GET /api/journey-rods/me/world`, a **read-only** projection. It deliberately does not call `evaluateJourneyRod()`, which can write rod events and Current Arcs. Stages are the scenario's gates, stage position is the persisted `current_stage`, and atoms are each gate's required molecules (direct and via clusters) carrying their latest evidence. Missing evidence stays a placeholder, and sensitive molecules show only "Captured". `journeyDefinitionFromPersistedRod()` in `src/lib/journeyEngine/genesis.js` adapts that projection into the same definition shape the Deal Journey uses, so there's no second rendering path. "Configure Deal" now renders the rod it actually saved instead of the `DEAL_JOURNEY_EXPERIENCE` template (the template is the fallback only). Rods with no resolvable scenario (career master, entitlements) are skipped. Anonymous visitors get a sign-in prompt instead of "Enter the World". The seeded rods stay because `OBJECTIVES` target their atoms. The homepage crystal city (`ProductHeroBlock` → `CrystalOfficeScene`) is not yet linked into this world.
