...
Data-model foundation (2026-08-06) for the "Salt Basin Weekly Research & Outreach Master Agent" spec — two non-collapsing pipelines (Salt Basin commercial opportunities, and career opportunities) run by a governed 8-agent roster. Built after a reuse-first audit via the `salt-basin-channel-journey-architecture` skill; see `server/lib/opportunityPipelineRegistry.js`'s header comment for the full classification. Phase 1 was data model + migrations only. Phase 2 (2026-08-06, same day) is a real, working career-pipeline vertical slice: `server/lib/careerOpportunityRollups.js` + `server/routes/careerPlacementAgents.js` (`/api/career-agents/*`, member-scoped via `requireUser`) + a real member-facing admin tab. Phase 3 (2026-08-06, same day) mirrors it for the commercial pipeline: `server/lib/commercialOpportunityRollups.js` + `server/routes/commercialOpportunities.js` (`/api/commercial-opportunities/*`, admin-scoped via `requireAdmin` — Salt Basin's own business development, not member self-service) + `src/components/admin/CommercialOpportunityPanel.jsx`, wired into the admin `crm` nav view alongside Leads. Deferred to a later phase: the actual research/scoring/outreach-drafting agent *execution* logic (today a member/admin manually tracks an opportunity and records its dimension scores; nothing runs unattended yet).

- **`src/components/OpportunityAgentOrbitWorld.jsx`** (renamed from `CareerAgentOrbitWorld.jsx` in Phase 3) is shared by both pipelines' panels via a `variant` prop selecting the `CRYSTAL_VARIANTS` anchor — `'agentHub'` (icosahedron core + gold/teal rings) for career, `'commercialPipeline'` (dodecahedron core + an ascending helix of Ring-0-5-evoking satellite gems) for commercial. One component, two crystal identities — never a forked copy per pipeline.
- **Commercial dimension Atoms** (`commercial_dim_trigger_strength`, etc., 7 total, `value_domain: '0-5'`) registered the same way as career's 8, deferred from Phase 1 since nothing consumed them until Phase 3's real scoring UI existed.
- **`commercial_opportunity_target` rods have no Tributary parent** (per Phase 1's audit — no natural "GTM rod" to provision from) — `commercialOpportunityRollups.js` resolves scope from the acting admin's `org_memberships` (org-scoped if they belong to one, else `user_id`-scoped directly), matching the same `entities`/`agent_definitions` flexibility built in Phase 1.

- **Career Placement Agents admin tab** (`src/components/admin/CareerPlacementAgentsPanel.jsx`, componentId `careerPlacementAgents`) — wired into `AdminShell.jsx`'s explicit-branch pattern (member-scoped, like `resume`/`careerMaster`) and into `defaultMemberConfig.js`'s `memberTabs`. An **existing** member's stored `member_configs` row predating this tab still gets it: `memberConfig.js`'s `GET /draft` does a read-time-only additive merge (any `defaultMemberConfig().navigation.memberTabs` entry missing by `id` gets appended, never reordering/overwriting what's already there) — never a persisted write to the member's row, honoring the never-write-member-rows-from-seed/bootstrap invariant while still surfacing new capabilities to existing members.
- **Crystal design system, not bespoke geometry** — per explicit direction, every user "world" is a variant of the one shared crystal family in `src/lib/crystalGeometry.js` (`CRYSTAL_VARIANTS`, `addCrystalLights`, `buildGemMesh` — "never fork a variant locally in a consuming component," that file's own header rule). Added `CRYSTAL_VARIANTS.agentHub` (a signature-family icosahedron core + gold/teal coordination rings) as the Agent Hub world's anchor crystal. `src/components/CareerAgentOrbitWorld.jsx` is a new, small, purpose-built Three.js scene (real `three@0.185` npm import, not the CDN-loaded r128 `SaltBasinCrystal.jsx` uses) built around that anchor: agents orbit as person figures (their own approved shape — agents are actors, not data), tracked opportunities orbit as `buildGemMesh()` gems colored/sized by real score. Selecting an orbit object does a brief camera dolly toward it (the "enter the object" transition) before the side panel opens. Deliberately does **not** touch `SpatialJourneyWorld.jsx` (2038 lines, zero props, hardwired to the Lonetree scene, no hierarchy/tree layout primitive) — retrofitting it was assessed and explicitly rejected as disproportionate risk for this phase.
- **`idx_rods_user_type` / `idx_rods_user_org_type` extended** — both partial unique indexes on `journey_data_rods` originally allowed only one rod per `(user_id, rod_type)` (with `member_entitlement` and `scenarioKey`-tagged rows as the only two carved-out exceptions). `commercial_opportunity_target`/`career_opportunity_target` need many independent, un-keyed rods per member (one per tracked opportunity), so both indexes now also exclude those two rod_types — found via a real constraint-violation bug hit while building the UI (a member's *second* tracked opportunity failed with `duplicate key value violates unique constraint`), not caught by Phase 1's single-opportunity test coverage.
- **Career scoring dimensions are registered Atoms**, not ad-hoc evidence keys — 8 `journey_metadata_molecules` rows (`career_dim_scope_altitude`, etc., `value_domain: '0-5'`), matching every other Atom-definition convention in this file. `careerOpportunityRollups.js`'s `recordDimensionScores()` upserts one `journey_rod_evidence` row per dimension (`ON CONFLICT (rod_id, molecule_key, source_reference) DO UPDATE`) and never fabricates a score: an opportunity with no recorded evidence reports `score: null`, not a guess.
...
### Resume Product (design-stage — intake only, not yet implemented)

A full requirements package for a "Salt Basin Resume Product" was supplied 2026-09-10 — driven by Betsy's real need to apply for jobs (starting with an actual Osaic application) using output the Salt Basin website itself produces. Preserved verbatim at `docs/baseline/intake/2026-09-10-salt-basin-resume-product/`, registered as evidence in `docs/baseline/01-source-register.md` (SRC-RESUME-01–14), candidate requirements in `docs/baseline/05-new-requirement-register.md` (NEW-018–031, all still `Pending` — none read back to Betsy for confirmation yet), and open decisions in `docs/baseline/07-decision-log.md` (DEC-006–010). A repeatable build driver lives at `.claude/skills/salt-basin-resume-product/SKILL.md` (`/salt-basin-resume-product`), a sibling to `salt-basin-pre-build`/`salt-basin-master-build`. **No schema, route, or UI code has been changed for this yet** — this section exists so the eventual build reuses the Career Channel Rod substrate below instead of reinventing it.

Key shape, per the supplied spec: a Career Master Foundation of user-attested career records (career facts originate only with the user — AI never supplies a missing biographical fact); an evidence-state vocabulary (user-attested / document-supported / externally verified / AI-proposed interpretation / disputed) that must never be blurred, especially "user approval of AI wording" vs. "independent verification"; three **distinct, never-merged** scoring concepts — the existing S03 §7 opportunity-ranking model (very likely already `careerOpportunityRollups.js` below — not yet confirmed by direct comparison), a proposed requirement-level transferable-skill-coverage rubric (75/50 thresholds), and Betsy's own dictated review-gating bands (≥90% ships without asking; 75–90% recommends review without blocking; <75% requires additional evidence or explicit human judgment before proceeding) — see DEC-006; and a dual factual-provenance/editorial-contribution ledger, conceptually adjacent to but distinct from the separate `salt-basin-contribution-intelligence` build (that one attributes *this platform's own* Claude-session work, not a member's personal career claims).
